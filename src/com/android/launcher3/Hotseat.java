...
public class Hotseat extends CellLayout implements Insettable {
        ...
        switch (style) {
            ...
            case HOTSEAT_STYLE_BLUR:
                ...
                if (mBlurRenderer == null) {
                    ...
                    mBlurRenderer.setCrossWindowBlurEnabled(true);
                    mBlurRenderer.setPreferSourceBlur(true);
                    if (isAttachedToWindow()) {
                        mBlurRenderer.onAttachedToWindow();
                        if (getParent() instanceof View) {
                            mBlurRenderer.setSourceView((View) getParent());
                        }
                    }
                } else if (isAttachedToWindow() && getParent() instanceof View) {
                    mBlurRenderer.setSourceView((View) getParent());
                }
    ...
    private void getIconAreaBounds(Rect outRect) {
        ...
        int bottom = icons.getBottom() + vPad;

        // The ShortcutAndWidgetContainer has intrinsic bottom padding for invisible labels.
        // Shrink the bottom bound by 6dp to perfectly center the icons vertically in the pill.
        bottom -= (int) (6 * getResources().getDisplayMetrics().density);
